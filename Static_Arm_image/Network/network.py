...
class PlanFromArmStateImage(nn.Module):
    def __init__(self, input_size, n_points, dof, activation=nn.ReLU()):
        ...
        self.mlp1 = MultiLayerPerceptron((256+input_size, 128, 256), 2, activation)  # 6
        self.norm1 = nn.BatchNorm1d(256)
        self.highway_layers = Highway(256, 10, activation)
        self.mlp2 = MultiLayerPerceptron((256, 256, 256), 3, activation)
        self.norm2 = nn.BatchNorm1d(256)
    ...
    def forward(self, worlds, x):
        # x1 = self.worldsNet(worlds)  # (worlds_batch_train, 256)
        x1 = self.downsample(worlds)  # (worlds_batch_train, 1, 16, 16)
        x1 = torch.flatten(x1)  # worlds_batch_train * 256
        x1 = x1.reshape(worlds.shape[0], -1)  # (worlds_batch_train, 256)

        # x: torch.Size([points_batch_train * worlds_batch_train, 2 * 3])
        x1 = x1.repeat_interleave(np.int(x.shape[0] / worlds.shape[0]), 0)
        # x1: torch.Size([points_batch_train * worlds_batch_train, 256])
        x = torch.cat((x1, x), 1)  # Merge two branches: batch*(256+2*dof)  torch.Size([50, 262])
