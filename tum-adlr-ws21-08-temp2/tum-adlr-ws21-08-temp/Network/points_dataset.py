...
class StartEndPointsDataset(Dataset):
    def __init__(self, number, dof, image, world_limits, proc):
        ...
        self.q_se = torch.cat((proc.preprocessing(self.start_points),
                                proc.preprocessing(self.end_points)),
                               1)
        self.q_es = torch.cat((proc.preprocessing(self.end_points),
                                proc.preprocessing(self.start_points)),
                               1)
        self.pairs = torch.cat((self.q_se,
                                self.q_es),
                               1) # (number, 2 * dof)
    ...
    def __getitem__(self, item):
        return self.start_points[item], self.end_points[item], self.q_se[item], self.q_es[item], self.pairs[item]
...
def sample_points(number, dof, image, world_limits):
    ...
    q_sampled = torch.FloatTensor(q_sampled)
    # q_sampled = q_sampled.cuda()
    return q_sampled
