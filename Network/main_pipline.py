...
training_data = StartEndPointsDataset(start_end_number_train - 5, par)
record_data_train = StartEndPointsDataset(5, par)
...
for epoch in range(101):
    ...
    for _, (start_points_train, end_points_train) in enumerate(train_dataloader):
        pairs_train = torch.cat((proc.preprocessing(start_points_train), proc.preprocessing(end_points_train)), 1)  # (number, 2 * dof)
        ...
        do_dq = weight[0] * length_cost.mean() * length_jac / np.sqrt(length_cost[:, None, None]) + weight[1] * collision_jac
        ...
        train_feasible += oc_check2(q.detach().numpy(), par.robot, par.oc, verbose=0).mean()
    ...
    with torch.no_grad():
        ...
        nurbs = NURBS(p=q_p, degree=degree, w=control_point_weights, u=u)
        q = nurbs.evaluate()
        q_full = torch.cat((record_data_train.start_points[:, None, :], q, record_data_train.end_points[:, None, :]), 1)
        if epoch % 5 == 0:
