...
class NURBS:
    def __init__(self, p, degree=3, k=None, w=None, u=None):
        ...
        if u is None:
            ...
            distance = dist(start_points, end_points)
            dist_min = distance.min()
            dist_max = distance.max()
            dst = dist_max - dist_min
            distance = (distance-dist_min)/dst
            ...
            for i in range(len(n_waypoints)):
                u_list.append(torch.linspace(0.01, self.n_points - 1 - 0.01, n_waypoints[i]))
            self.u = u_list  # Todo
            '''
