...
for epoch in range(101):
    ...
    for _, (start_points_train, end_points_train, pairs_train) in enumerate(train_dataloader):
        ...
        for i in range(control_point_weights.shape[0]):
            dist = torch.nn.PairwiseDistance(p=2)
            distance = dist(start_points_train[i], end_points_train[i])
            distance = distance / 10   # 10 meter

            n_waypoints = torch.ceil(15 * distance).int()
            u_i = torch.linspace(0, 1, n_waypoints)

            nurbs = NURBS(p=control_points[i], degree=degree, w=control_point_weights[i])
            q_i = nurbs.evaluate(u_i)
            # q_i = torch.moveaxis(q_i, 1, 0).numpy()
            # tensor.detach().numpy()

            print("q_i", q_i)

            # q = torch.zeros(15, 2)
            q = torch.stack((q, q_i), 0)  # Todo [50,15,2]

            # dq_dp_i = nurbs.evaluate_jac(u_i)  # Todo
            # dq_dp =
        print("q", q)
