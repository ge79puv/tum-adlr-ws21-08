...
for i in range(world_number - test_number):
    initialize_oc(oc=par.oc, world=par.world, robot=par.robot, obstacle_img=np.squeeze(images[i].numpy()))  # Todo
    ...
    for epoch in range(11):  # Todo  101
        # training
        model.train()
        train_loss, train_feasible = 0, 0
        for _, (start_points_train, end_points_train, pairs_train) in enumerate(train_dataloader):

            control_points, control_point_weights = model(pairs_train, images.index_select(0, torch.tensor([i])))
        ...
        with torch.no_grad():
            control_points, control_point_weights = model(record_data_train.pairs, images.index_select(0, torch.tensor([i])))
            nurbs = NURBS(p=control_points, degree=degree, w=control_point_weights)
            q = nurbs.evaluate()
            q_pp = proc.postprocessing(q)
            q_full = torch.cat((record_data_train.start_points[:, None, :], q_pp, record_data_train.end_points[:, None, :]), 1)
            if epoch % 5 == 0:
                name = 'world' + str(i) + '_record_train_epoch_' + str(epoch)
                plot_paths(q_full, par, 5, name, plot_path1, save=save_image)

            for _, (start_points_val, end_points_val, pairs_val) in enumerate(val_dataloader):
                control_points, control_point_weights = model(pairs_val, images.index_select(0, torch.tensor([i])))
                nurbs = NURBS(p=control_points, degree=degree, w=control_point_weights)
                q = nurbs.evaluate()
                q_pp = proc.postprocessing(q)
    ...
    np.save("train_feasible" + str(i), train_feasible_history)
    np.save("val_feasible" + str(i), val_feasible_history)

model.load_state_dict(torch.load("model"))   # Todo???
model.eval()
